...
void ModbusSlaveESP::loop() { read_uart(); }
...
// Handle the function code Read Input Status (FC=02) and write back the values from the digital input pins (discreet
// input).
uint8_t readDigitalIn(uint8_t fc, uint16_t address, uint16_t length) { return STATUS_OK; }
