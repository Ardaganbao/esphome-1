...
bool ModbusBase::parse_modbus_byte_(uint8_t byte) {
  ...
  ESP_LOGW(TAG, "arda-ModbusBase recieved Byte  %d (0X%x)", byte, byte);
  ...
  if (function_code == 0x5 || function_code == 0x06 || function_code == 0x10|| function_code == 0x03|| function_code == 0x04) {
    data_offset = 2;
    data_len = 4;
  }
